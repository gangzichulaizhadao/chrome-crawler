...
function shanghai(request: any, currentPage: number): any {
  const { url, method, body, headers } = JSON.parse(JSON.stringify(request))
  // qs转换成对象
...
function zhejiang(request: any, currentPage: number): any {
  return shanghai(request, currentPage)
}
...
export function handleRequestData1(capturedRequests: any, currentPage: number, item: any) {
  const { url, method, body, headers } = JSON.parse(JSON.stringify(capturedRequests))
  // qs转换成对象
  let parsedBody = qs.parse(body)
  parsedBody.data = JSON.parse(parsedBody.data)
  // 处理请求参数
